				...
				if( j == MAX_SLOTS &&
				   ...
				   items[i].bound == item.bound )
					...
					StringBuf_Printf(&buf, "UPDATE `%s` SET `amount`='%d', `equip`='%d', `identify`='%d', `refine`='%d',`attribute`='%d', `expire_time`='%u', `favorite`='%d', `bound`='%d'",
					    charserv_table(inventory_table), items[i].amount, items[i].equip, items[i].identify, items[i].refine, items[i].attribute, items[i].expire_time, items[i].favorite, items[i].bound);
					for( j = 0; j < MAX_SLOTS; ++j )
						StringBuf_Printf(&buf, ", `card%d`=%hu", j, items[i].card[j]);
					StringBuf_Printf(&buf, " WHERE `id`='%d' LIMIT 1", item.id);
...
int char_delete_char_sql(uint32 char_id){
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `type`=3 AND `char_id`='%d'", charserv_table(reg_table), char_id) )
		Sql_ShowDebug(sql_handle);
...
bool char_checkdb(void){
	...
	const char* sqltable[] = {
		...
		charserv_table(auction_table), charserv_table(reg_table), charserv_table(ragsrvinfo_table), charserv_table(interlog_table),
	};
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `time`,`char_msg`,`account_id`,`char_num`,`name`,"
		...
		" FROM `%s`;", charserv_table(charlog_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking reg_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`str`,`value`,`type`,`account_id`"
		" FROM `%s`;", charserv_table(reg_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`hotkey`,`type`,`itemskill_id`,`skill_lvl`"
		" FROM `%s`;", charserv_table(hotkey_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `account_id`,`char_id`,`type`,`tick`,`val1`,`val2`,`val3`,`val4`"
		" FROM `%s`;", charserv_table(scdata_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`id`,`lv`,`flag`"
		" FROM `%s`;", charserv_table(skill_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `time`,`log`"
		" FROM `%s`;", charserv_table(interlog_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking memo_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `memo_id`,`char_id`,`map`,`x`,`y`"
		" FROM `%s`;", charserv_table(memo_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking guild_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `guild_id`,`name`,`char_id`,`master`,`guild_lv`,"
		...
		" FROM `%s`;", charserv_table(guild_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `guild_id`,`opposition`,`alliance_id`,`name`"
		" FROM `%s`;", charserv_table(guild_alliance_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `castle_id`,`guild_id`,`economy`,`defense`,`triggerE`,"
		...
		" FROM `%s`;", charserv_table(guild_castle_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `guild_id`,`account_id`,`name`,`mes`"
		" FROM `%s`;", charserv_table(guild_expulsion_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `guild_id`,`account_id`,`char_id`,`hair`,"
		...
		" FROM `%s`;", charserv_table(guild_member_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `guild_id`,`id`,`lv`"
		" FROM `%s`;", charserv_table(guild_skill_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `guild_id`,`position`,`name`,`mode`,`exp_mode`"
		" FROM `%s`;", charserv_table(guild_position_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `party_id`,`name`,`exp`,`item`,`leader_id`,`leader_char`"
		" FROM `%s`;", charserv_table(party_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `pet_id`,`class`,`name`,`account_id`,`char_id`,`level`,"
		...
		" FROM `%s`;", charserv_table(pet_table)) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking friend_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`friend_account`,`friend_id` FROM `%s`;", charserv_table(friend_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking mail_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `id`,`send_name`,`send_id`,`dest_name`,`dest_id`,"
			...
			" FROM `%s`;", charserv_table(mail_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking auction_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `auction_id`,`seller_id`,`seller_name`,`buyer_id`,`buyer_name`,"
			...
			" FROM `%s`;", charserv_table(auction_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking quest_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`quest_id`,`state`,`time`,`count1`,`count2`,`count3` FROM `%s`;", charserv_table(quest_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `homun_id`,`char_id`,`class`,`prev_class`,`name`,`level`,`exp`,`intimacy`,`hunger`,"
			...
			" FROM `%s`;", charserv_table(homunculus_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `mer_id`,`char_id`,`class`,`hp`,`sp`,`kill_counter`,`life_time` FROM `%s`;", charserv_table(mercenary_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`merc_id`,`arch_calls`,`arch_faith`,"
			...
			" FROM `%s`;", charserv_table(mercenary_owner_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `ele_id`,`char_id`,`class`,`mode`,`hp`,`sp`,`max_hp`,`max_sp`,"
			...
			" FROM `%s`;", charserv_table(elemental_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking ragsrvinfo_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `index`,`name`,`exp`,`jexp`,`drop` FROM `%s`;", charserv_table(ragsrvinfo_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking skillcooldown_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `account_id`,`char_id`,`skill`,`tick` FROM `%s`;", charserv_table(skillcooldown_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking bonus_script_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `char_id`,`script`,`tick`,`flag`,`type`,`icon` FROM `%s`;", charserv_table(bonus_script_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `id`,`char_id`,`nameid`,`amount`,`equip`,`identify`,`refine`,"
		...
		" FROM `%s`;", charserv_table(cart_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `id`,`char_id`,`nameid`,`amount`,`equip`,`identify`,`refine`,"
		...
		" FROM `%s`;", charserv_table(inventory_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking storage_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `id`,`account_id`,`nameid`,`amount`,`equip`,`identify`,`refine`,"
		"`attribute`,`card0`,`card1`,`card2`,`card3`,`expire_time`,`bound`,`unique_id`"
		" FROM `%s`;", charserv_table(storage_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `id`,`guild_id`,`nameid`,`amount`,`equip`,`identify`,`refine`,"
		"`attribute`,`card0`,`card1`,`card2`,`card3`,`expire_time`,`bound`,`unique_id`"
		" FROM `%s`;", charserv_table(guild_storage_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
static void char_schema_config_init(void) {
	...
	schema_config.reg_table				 = StringBuf_FromStr("global_reg_value");
...
static void char_schema_config_final(void) {
	...
	StringBuf_Free(schema_config.reg_table);
